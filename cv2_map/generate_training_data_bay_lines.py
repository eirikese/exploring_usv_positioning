...
num_points_per_line = 50 # 50 for bay_lines
noise_std = 0 # 1 for bay_lines
N = 4
scaling_factor = 1  # Scaling factor (meters = 1, m_to_cm = 100) # 1 for bay_lines
...
def visualize_images(output_dir, num_images, transformations):
...
if __name__ == "__main__":
    ...
    scaled_and_centered_points = center_and_scale_points(original_noisy_points, scaling_factor, image_size)
    ...
    for i in range(N):
        ...
        image = create_image_from_point_cloud(transformed_points, image_size)
        save_data(transformed_points, image, i, output_dir)
        transformations.append((translation, angle))

    print(f"Saved {N} datasets to {output_dir}")
    visualize_images(output_dir, N, transformations)  # Display images with transformations