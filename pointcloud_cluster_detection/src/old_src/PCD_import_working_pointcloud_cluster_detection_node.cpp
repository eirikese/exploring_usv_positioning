...
#include <pcl/io/pcd_io.h>  // Include PCL's PCD I/O header
...
public:
  ...
  // Add this function to load and publish the object
  void loadAndPublishObject(const std::string& pcd_file_path)
  {
    // Load the 3D object from the PCD file
    pcl::PointCloud<pcl::PointXYZ>::Ptr loaded_object(new pcl::PointCloud<pcl::PointXYZ>);
    if (pcl::io::loadPCDFile<pcl::PointXYZ>(pcd_file_path, *loaded_object) == -1)
    {
    ...
    pcl::toROSMsg(*loaded_object, object_msg);

    // Set the frame ID to "os_sensor" (or the appropriate frame name)
    object_msg.header.frame_id = "os_sensor"; // Update with the correct frame name

    // Publish the loaded object
    object_pub_.publish(object_msg);

    ROS_INFO("Loaded object published to /loaded_object topic.");
  }
...
int main(int argc, char** argv)
{
  ...
  while (ros::ok())
  {
    // Load and publish the PCD file
    pcl_cluster_detection.loadAndPublishObject("/home/eirik18/lidar_ws/src/pointcloud_cluster_detection/objects/testthingy60cm.pcd");

    // Sleep to control the publication rate
    rate.sleep();
  }
